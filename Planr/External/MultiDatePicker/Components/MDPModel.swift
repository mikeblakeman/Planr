...
class MDPModel: NSObject, ObservableObject {
    ...
    func dayOfMonth(byDay: Int) -> MDPDayOfMonth? {
        ...
        for dom in days where dom.day == byDay {
            return dom
        }
        return nil
    }
...
extension MDPModel {
    ...
    private func buildDays() {
        ...
        for iterator in 0..<numDays {
            let realDate = calendar.date(from: DateComponents(year: year, month: month, day: iterator+1))
            var dom = MDPDayOfMonth(index: index, day: iterator+1, date: realDate)
